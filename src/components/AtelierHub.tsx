...
type AtelierMode = "hub" | "phrase" | "script" | "diagnostic" | "case";
...
export function AtelierHub({ addXp }: AtelierHubProps) {
  const [mode, setMode] = useState<AtelierMode>("hub");

  if (mode === "phrase") return <div><BackButton onClick={() => setMode("hub")} /><PhraseLab addXp={addXp} /></div>;
  if (mode === "script") return <div><BackButton onClick={() => setMode("hub")} /><ScriptBuilder addXp={addXp} /></div>;
  if (mode === "diagnostic") return <div><BackButton onClick={() => setMode("hub")} /><DiagnosticBuilder addXp={addXp} /></div>;
  if (mode === "case") return <div><BackButton onClick={() => setMode("hub")} /><CaseCreator addXp={addXp} /></div>;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {ATELIERS.map((a, i) => (
          <motion.button
            ...
            className={`rounded-2xl bg-gradient-to-br ${a.color} border ${a.border} p-6 text-left transition-all group`}
          >
            <div className="text-3xl mb-3 group-hover:scale-110 transition-transform">{a.icon}</div>
            <div className="text-base font-bold tracking-tight">{a.title}</div>
            <div className="text-xs text-muted-foreground mt-1">{a.desc}</div>
            <div className="mt-3 text-[10px] font-semibold text-foreground/50 group-hover:text-foreground/70 transition-colors">
              Commencer →
            </div>
          </motion.button>
        ))}
